...
var Vm = new Vue({
    ...
    data: {
        ...
        address: 'ws://127.0.0.1:4321', // 链接地址
        alert: {
            class: 'success',
            state: false,
            content: '',
    ...
    methods: {
		...
		isPicBase64:function(jsonString) {  
            try {  
                const data = JSON.parse(jsonString);  
				console.log(data);
                return data.hasOwnProperty('pic_base64');  
            } catch (e) {  
                // 处理JSON解析错误  
		...
		getBase64Image:function(jsonString) {  
            try {  
                const data = JSON.parse(jsonString);  
                return `data:image/jpeg;base64,${data.pic_base64}`; // 假设是jpeg图片，根据实际情况修改  
            } catch (e) {  
                // 处理JSON解析错误  
                console.error('Invalid JSON:', e);  
                return ''; // 返回一个空字符串或默认图片URL  
            }  
        },
		isPic:function(jsonString) {  
            try {  
                const data = JSON.parse(jsonString);  
				console.log(data);
                return data.hasOwnProperty('pic');  
            } catch (e) {  
                // 处理JSON解析错误  
                console.error('Invalid JSON:', e);  
                return false;  
		...
		getImage:function(jsonString) {  
            try {  
                ...
                const data = JSON.parse(jsonString);  
                return `${data.pic}?version=2${guid}`; 
            } catch (e) {  
                // 处理JSON解析错误  
                console.error('Invalid JSON:', e);  
                return ''; // 返回一个空字符串或默认图片URL  
